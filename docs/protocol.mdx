---
title: "Protocol"
description: "Detailed specification of the Op/Evt message protocol"
---

Ante's client-daemon communication is built on a typed message-passing protocol. Operations (`Op`) flow from client to daemon, and events (`Evt`) flow from daemon to client.

## Core concepts

| Concept | Description |
|---|---|
| **Project** | A git repo or root directory. Can have multiple sessions. |
| **Session** | One episode of interaction between user and Ante. |
| **Task** | One piece of work the user wants to accomplish. Can span multiple turns. |
| **Turn** | One back-and-forth with the agent. Starts with user input, ends with agent message or approval request. |
| **Step** | One interaction from agent with LLM. Handles tool calls and other mechanics. |

<Note>
  Generally, if there is no approval interruption, one task consists of one turn.
</Note>

## Basic UI flow

A single user input followed by a 2-turn task:

```mermaid
sequenceDiagram
  autonumber
  box "UI"
    participant User
  end
  box "Daemon"
    participant Core
    participant Session
    participant Task
  end
  box "LLM Provider"
    participant LLM
  end

  User->>Core: Op::ConfigureSession
  Core->>Session: Create session
  Core-->>User: Evt::SessionInitialized

  User->>Session: Op::UserInput
  Session-->>+Task: Start task
  Task-->>User: Evt::TaskStarted

  rect rgb(245,245,245)
    note over Task,LLM: Turn 1
    Task->>LLM: prompt
    LLM-->>Task: response (exec)
    Task-->>User: Evt::ExecApprovalRequest
    User->>Task: Op::ExecApproval::Allow
    Task-->>User: Evt::ExecStart
    Task->>Task: exec
    Task-->>User: Evt::ExecStop
    Task-->>User: Evt::TurnComplete
  end

  rect rgb(245,245,245)
    note over Task,LLM: Turn 2
    Task->>LLM: stdout
    LLM-->>Task: response (patch)
    Task->>Task: apply patch (auto-approved)
    Task->>LLM: success
    LLM-->>Task: response (msg + completed)
    Task-->>User: Evt::AgentMessage
    Task-->>User: Evt::TurnComplete
  end

  Task-->>-User: Evt::TaskComplete
```

## Task interruption

Interrupting a task and continuing with additional user input:

```mermaid
sequenceDiagram
  autonumber
  box "UI"
    participant User
  end
  box "Daemon"
    participant Session
    participant Task1
    participant Task2
  end
  box "LLM Provider"
    participant LLM
  end

  User->>Session: Op::UserInput
  Session-->>+Task1: Start task
  Task1-->>User: Evt::TaskStarted
  Task1->>LLM: prompt
  LLM-->>Task1: response (exec)
  Task1->>Task1: exec (auto-approved)
  Task1-->>User: Evt::TurnComplete
  Task1->>LLM: stdout
  LLM-->>Task1: response (exec)
  Task1->>Task1: exec (auto-approved)
  User->>Task1: Op::Interrupt
  Task1-->>-User: Evt::Error("interrupted")

  User->>Session: Op::UserInput w/ last_response_id
  Session-->>+Task2: Start task
  Task2-->>User: Evt::TaskStarted
  Task2->>LLM: prompt + Task1 last_response_id
  LLM-->>Task2: response (exec)
  Task2->>Task2: exec (auto-approved)
  Task2-->>User: Evt::TurnComplete
  Task2->>LLM: stdout
  LLM-->>Task2: msg + completed
  Task2-->>User: Evt::AgentMessage
  Task2-->>User: Evt::TurnComplete
  Task2-->>-User: Evt::TaskCompleted
```

## Operations reference

| Op | Fields | Description |
|---|---|---|
| `NewSession` | model, provider, policy, streaming, config | Initialize a new session |
| `UserInput` | String | Submit a user prompt |
| `ApprovalResponse` | allow/deny | Respond to tool approval request |
| `SlashCommand` | command name, args | Execute a custom command |
| `OfflineMode` | OfflineModeOp | Offline mode operations |
| `Interrupt` | — | Abort the current task |
| `Shutdown` | — | Clean shutdown |

## Events reference

| Evt | Fields | Description |
|---|---|---|
| `SessionInit` | metadata | Session is ready |
| `TaskStarted` | id | A new task has begun |
| `TaskFinished` | id, error, is_interrupted | Task completed or failed |
| `AgentMessage` | String | Text response from agent |
| `Thinking` | String | Chain-of-thought content |
| `MessageDelta` | String | Streaming content chunk |
| `ToolCallStarted` | tool_use | Tool execution began |
| `ToolCallFinished` | result | Tool execution completed |
| `ToolCallCancelled` | — | Tool execution was cancelled |
| `RequestApproval` | tool_use | Agent needs permission |
| `UsageUpdate` | tokens, cost | Token/cost tracking |
| `Info` | String | Informational message |
| `Error` | String | Error message |
